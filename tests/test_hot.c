...
static unsigned long run(const char *path, int threads, int profile, float *out) {
    ...
    if (s != NULL) {
        ...
        for (int i = 1; i < N_TOKENS - 4; i++) TR_CHECK(tr_session_eval(s, tok + i, 1) == 0);
        TR_CHECK(tr_session_eval(s, tok + N_TOKENS - 4, 4) == 0);
...
int main(int argc, char **argv) {
    ...
    printf("  %d tokens, pools of 1/2/3/8 threads, profiler off/on: no allocations, identical logits\n", N_TOKENS);
    remove(path);
    TR_TEST_EXIT();
}
