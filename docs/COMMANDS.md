...
AB_ZONE_PHASE=decode AB_ZONE_N=64 AB_ZONE_THREADS=8 sh tools/ab_zone.sh <before> <after> expert_gate_up+expert_down 8 "512 2048"   # the same on the decode's zones (a + sums several), its width forced
...
sh tools/race_q4k.sh [runs] [m]   # by the native rules: the real model in Q4_K against Q8_0, and llama.cpp on the Q4_K; with m, Q4_K_M against Q4_K (~20 min); RACE_THREADS=4 for other thread counts (default 16,8)
sh tools/race_smt.sh [runs]   # container: Trochilus and llama.cpp under the same taskset (SMT_CPUS=0-7, SMT_THREADS=4,8, SMT_ENV for an A/B); the container's siblings are not SMT (LESSONS #184): SMT natively with start /affinity (MEASUREMENTS §SMT in the decode)
make BUILD=build/win-trace EXTRA_CFLAGS=-DTR_POOL_TRACE build/win-trace/trochilus.exe   # the pool traced: TR_POOL_TRACE_FILE=x.trace on a run, then tools/.venv/Scripts/python.exe tools/pool_trace.py x.trace (each call shape's wall, work, tail; MEASUREMENTS §The pool's tail)
tools/.venv/Scripts/python.exe tools/token_timeline.py x.trace --bw 53   # a decode token's timeline from the same trace: each call's bytes (TR_TRACE_NOTE), wall, GB/s and loss against a plain read at --bw; the RAM's idle share of a token (MEASUREMENTS §The engine read as entangled pairs)
sh tools/prompt_scale.sh build/ps 8 16   # the traced prompt pass (2048 tokens, Q4_K) at each width; PS_BIN=build/linux-trace/trochilus in the container, TR_POOL_PIN=0 without the pin
tools/.venv/Scripts/python.exe tools/prompt_timeline.py build/ps/trace_t8.txt build/ps/trace_t16.txt --ccd 8   # per call type: work, tails, gaps, and the loss against a perfect scaling (MEASUREMENTS §The prompt from 8 to 16 threads)
sh tools/bench_q4k_genome.sh [runs] [args]   # the decode's Q4_K dot sequenced in L1 (question 66): the cost of each instruction on this core, the kernel with one piece removed at a time, the candidates bit for bit against scalar; container by default, NATIVE=1 native; args: --only <name part> (tier and those kernels, no instruction lines), --smt (two threads on one core's siblings, native), --ram T / --ram-smt C (~1 GiB from RAM, T threads or 2 on each of C cores, with the plain read), --dyn G (also rows claimed G at a time) (~1-6 min, most of it waiting for a still machine)
tools/.venv/Scripts/python.exe tools/q4k_tables.py <model.gguf> [--tensors <regex>]   # distinct (scale, min) pairs of the Q4_K sub-blocks per tensor, and how many the 16384 most common cover (question 66, premise e)
...
make draft-probe   # build/draftprobe/draft_probe (in container: BUILD=build/linux-gcc): how often a draft inside the exact bits (Q8_0 high nibble, KV high 16 bits; variants with 4 experts, a KV window, the previous token's top positions with and without their successors, the history from a copy of 8 or 4 bits) picks the exact token from the exact state, and its second choice; --text 1 follows the real text. Then tools/draft_probe_report.py build/entangled/probe_*.tsv: agreement by the exact margin, rep4, passes simulated (fixed k, or stopped by the draft's own margin; with and without the second choice as one more leaf, in sample and out of sample: oos), a 16k-row draft head (+h16k), MiB a token against llama.cpp
...
build/tests/bench_mem.exe gateup --runs 21   # a decode's gate, up and activation from RAM: three calls, one call on two streams, one on rows interleaved at load, without the activation, a plain read; alternated every repetition
