...
# Output: ref/llama.cpp/build-trochilus/bin/{llamacpp_logits,llama-tokenize,llama-bench,llama-quantize}
set -e
...
cmake --build "$B" --target llama llama-tokenize llama-bench llama-quantize -j "$(nproc)" >/dev/null
mkdir -p "$B/bin"
...
ls -la "$B/bin/llamacpp_logits" "$B/bin/llama-tokenize" "$B/bin/llama-bench" "$B/bin/llama-quantize"
}
main "$@"; exit
