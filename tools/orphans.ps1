...
$scripts = 'tools[/\\](prefill_context|decode_context|threads_phase|remeasure|ab_modes|ab_speed|ab_spec|platform_bits|busy_machine)\.sh|tools[/\\]profile_suite\.py'
$all = Get-CimInstance Win32_Process
$byId = @{}
foreach ($p in $all) { $byId[[int]$p.ProcessId] = $p }
$mine = @{}
# the callers, as MSYS sees them (tools/orphans.sh): Windows loses the chain at every MSYS exec
foreach ($c in ($Callers -split ',')) { if ($c -match '^\d+$') { $mine[[int]$c] = $true } }
$id = $PID
while ($id -and $byId.ContainsKey([int]$id) -and -not $mine.ContainsKey([int]$id)) {
    $mine[[int]$id] = $true
