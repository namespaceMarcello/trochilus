...
greedily from the same prompt. Prints, over the positions: how many rows are the same bytes, the
largest difference between two logits, the mean and the largest KL(A || B), how many positions
choose another token; and how many generated tokens are the same before the first difference.
Used by tools/expf_quality.sh (docs/MISURE.md question 37).
"""
import sys
...
def main():
    ...
    if a.shape != b.shape:
        ...
        return 1
    same_rows = int((a.view(np.uint32) == b.view(np.uint32)).all(axis=1).sum())
