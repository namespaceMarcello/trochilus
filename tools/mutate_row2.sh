#!/bin/sh
# mutate_row2.sh — do the tests see a wrong two-row kernel (dot_row2_x4: two weight rows against the
# same four input rows, AVX-512), a wrong use of it in tr_matmul, or a wrong SIMD Q6_K scale? A test
...
EOF
# $1: name, $2: file, $3: text, $4: replacement (the first occurrence)
run() {
