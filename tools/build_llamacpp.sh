...
# Output: ref/llama.cpp/build-trochilus/bin/{llamacpp_logits,llama-tokenize}
set -e
...
cmake --build "$B" --target llama llama-tokenize -j "$(nproc)" >/dev/null
mkdir -p "$B/bin"
...
ls -la "$B/bin/llamacpp_logits" "$B/bin/llama-tokenize"
