...
|---|---|---|---|---|---|---|
| 1 | CPU matmul, the prompt (Q8_0, K-quants) | one row × one token, (expert, row chunk) items kept in cache; F32 activations with FMA, int8 opt-in (read 09-17, `xf_moe_run` 09-24) | one row against 2 tokens, **int8 activations** (read 09-17, activations 09-24) | read 09-24 (MEASUREMENTS §The prompt's matmul against the four references): int8 activations everywhere; tinyBLAS 4 × 4 for the dense weights, Q8_0 experts one dot at a time, Q4_K repacked 16 rows × 4 tokens; ik converts 32 rows at a time once, then one gemm over every token | their kernels alone on bench_peak's shapes (`tools/bench_ggml.sh`, container, loaded): **Q8_0 experts ours 1.42×** (102 against 72 a core); Q8_0 dense theirs 1.6× (tinyBLAS, int8); Q4_K repacked theirs 1.26× (experts) to 2.3× (dense). ik's decode-once kept exact (question 63): Q8_0 1.00–1.04×, Q4_K 1.1×, closed. x8 native (09-25): prefill 1.13–1.19×, logits identical. Engine race after x8 (09-25): **prefill level at 8 threads** (theirs 1.01–1.03×), theirs 1.30–1.35× at 16. More rows per input load (question 64, 4 × 6 and 3 × 8): lose 6–16%, closed as no **Phase-major (09-26, MEASUREMENTS §Phase-major)**: the exact definition run with 16 weight rows as SIMD lanes, 164.7 GFLOP/s a core (98.6% of the no-FMA peak; their repacked Q4_K 194 dense and 107 through `mul_mat_id`, int8); the engine natively: Q4_K prompt at 4 threads 140 → 218-223 tok/s (theirs 222 in the container), Q8_0 at 16 threads 390-419 → 550-569, the logits identical | **rebuilt 2026-09-26**: phase-major (the lane contract as a chain in time, the thinker's idea); the race in the container owed |
| 2 | CPU matmul, the decode (one token, memory-bound) | no separate one-token path on x86: one row × one vector, **int8 activations** quantized once per token, `dpbusd` (AVX-512 VNNI) or `maddubs` (AVX2), OpenMP static split of the rows; MoE: gate and up fused per (expert, row chunk) (`quant.h` `matmul_q_idot`, `expert_ffn.h` `xf_moe_run`; read 09-25) | **no x86 SIMD** in the CPU path: scalar C (NEON only on ARM), activations Q8_K; static equal split over a pthread pool (`ds4_parallel_for`), a decode-only MoE routine (`layer_routed_moe_one_prealloc`; read 09-25) | llama.cpp: tinyBLAS off at one column (`llamafile_sgemm` returns at n < 2), so `vec_dot` one row at a time, activations quantized once per matmul (Q8_0 / Q8_K) and shared; rows claimed in chunks of 64 by an atomic counter; repacked Q4_K runs `gemv` 8 rows at a time; `mul_mat_id` per expert, gate and up separate. ik: `nrc_y = 1` kernels, two independent block chains per row, static row split, **gate and up fused** per expert (read 09-25) | their kernels against ours in one binary, from RAM (`tools/bench_ggml_decode.sh`, container, MEASUREMENTS §The decode's matmul against ggml's): **Q8_0 level from 4 threads** (0.97-1.06x, both at the ceiling), theirs 1.4-1.7x at one; **Q4_K theirs 1.34-1.40x at 4 threads** until question 66 (MEASUREMENTS §The Q4_K decode dot sequenced): our scalar scale decode was 24% of a row; the scales in a vector one block ahead and two rows a call (`dot_row2`), exact, make it **1.04x (1.01-1.04)**, 12.5 GB/s a core, and the real model's decode at 4 threads **level with llama.cpp** (50.4 against 50.2 tok/s). Their 64-row atomic claiming tried in our decode: 0.93-0.97x | **closed 2026-09-25, reopened by question 66 the same evening**: Q4_K level at 4 threads within the exact definition; left: their 2x a core (int8 activations, question 60) and SMT (+9-13% from RAM at 4 cores, not wired) |
| 3 | Attention, prompt and decode, and the KV layout | MLA "absorb": K and V one latent row, F32 `[position][dim]` a layer (u8 codecs opt-in); decode: two-pass softmax, (token, head) pairs over OpenMP, the positions never split, no prefetch; the value axpy kept bit-exact, the score dot reassociated; prompt: tiled online softmax only above 2 GB of reconstruction, which it says rounds otherwise (`colibri.c` 4711-4919; read 09-25) | F32 `[position][dim]`, K and V the same row (MLA); on x86 the CPU attention is scalar (NEON only), two-pass softmax with a sink logit, one token after another, no threads: the oracle, CUDA is the engine (`ds4.c` 12579-12617, 15471; read 09-25) | llama.cpp: KV **F16** `[position][head × dim]` a layer; flash attention on by default on the CPU (AUTO resolves on); the decode at ≥ 512 positions **splits the positions over every thread**, each thread runs every head over its chunk (a 256-byte slice every 4 KiB, 16 strided passes), online softmax, **V summed in an F16 accumulator rounded at every position**, partials merged by rescaling (`ops.cpp` 8700-8810, 9261-9290); prompt: 64 × 64 tiles, masked positions skipped one by one. ik: FA on by default (`flash_attn = true`), the positions split for MLA only (`iqk_flash_attn.cpp` 373-423) (read 09-25) | the race of 09-25 (`f6a8b4c`, container, Q8_0) as a line, decode per token = intercept + slope × context (MEASUREMENTS §The attention against llama.cpp's): at 8 threads **ours 28.1 ms + 5.31 µs a position, theirs 27.6 + 3.24**; at 16, 26.8 + 5.24 against 26.8 + 3.34. Level at no context; the whole gap is the slope: we read 256 KiB a position at 49 GB/s, they read 128 KiB at 40. Ours at the speed of a plain read of its bytes at 4-8 threads (`bench_attn_bw`); one front instead of T refuted (`bench_mem streams`, 0.78-0.81x in the attention). Prompt: the products at 70 GFLOP/s a core, the lane trees in scalar; tiles of 4 queries x 4 positions **1.32x on the attention at 2048, 1.38x at 4000 on one core; the engine's zone 1.31-1.33x (prefill 1.03x at 2048, 1.06x at 4000)**, logits identical (MEASUREMENTS §The attention against llama.cpp's, §The prompt's attention in tiles) | **read and measured 2026-09-26**: the decode is at the RAM's ceiling, and passing F16 exactly would need 81 GB/s (the GPU attention passes it); the prompt's tiles built; open: the softmax's exponential in a vector (row 4, question 58), now half of a prompt's pair |
| 4 | Softmax and exp | the C library's `expf`, one value at a time (read 09-24) | the same (read 09-24) | `ggml_v_expf` (ARM's routine) in softmax and SiLU, sum in double after a lane reduce; the C library's `expf` in the CPU flash attention; ik the same routine (read 09-24) | every float (MEASUREMENTS §Softmax and exp against the references): ggml's differs from the exact exp on 3.36% (≤ 2 ulp, 0.16 ns), glibc's on 0.004% (1 ulp, 3.2 ns); ours 0, 3.5 ns scalar, 0.73 ns SIMD (not wired) | **read and measured**: exact at ≤ ~2% of a prompt; nothing to take |
